...
#include "Components/HealthComponent.h" // Include AnimatorSystem to control weights
...
public:
    ...
    void OnReassignation() override {
        targetEntity = Engine::ECS::NULL_ENTITY;
        ...
        if (funcSys) {
            ...
            TerminalInstance->error("FunctionRegistrySystem not found!");
            return;
        }
        

        // --- FIX: Reset persistent state for the new scene ---
        animationsInitialized = false; 
        currentMoveState = 0.0f;
        TerminalInstance->info("OnReassignation called.");
    }
        ...
        if (registry->HasComponent<Engine::Components::Transform>(entityID) && 
            ...
            if (isMouseCaptured) {
                ...
                if (InputSysteminstance->GetKeyPressed(GLFW_KEY_G)) {
                    engine->GetSystem<Engine::Systems::SceneSerializerSystem>()->RequestSceneDeserialization("Assets/Scenes/DefaultScene.pscene");
                }
            }
