...
namespace Engine::Systems {
    ...
    class ENGINE_API ImGuiSystem : public Core::ISystem {
    public:
        ...
        // --- NEW: UI Callback Registry ---
        // Expose a way for scripts to register their ImGui draw functions
        void RegisterUICallback(const std::string& id, std::function<void()> callback);
        void UnregisterUICallback(const std::string& id);
    ...
    private:
        ...
        ECS::Entity m_EntityToCopy;

        // The hash map storing the registered callbacks
        std::unordered_map<std::string, std::function<void()>> m_UICallbacks;
        ...
        std::string m_animEdFilePath = "";
        Engine::Assets::AssetsUtils::CPUAnimationData m_animEdData;
        bool m_animEdLoaded = false;
        int m_animEdSelectedEventIdx = -1;
        int m_animEdDraggingEventIdx = -1;
        float m_animEdPlayhead = 0.0f;

        // Preview Entity State
        std::optional<ECS::Entity> m_animEdPreviewEntity;
        std::string m_animEdMeshPath = "";
        std::string m_animEdSkeletonPath = "";
        glm::vec3 m_animEdPreviewPos{0.0f, 0.0f, 0.0f};
        glm::vec3 m_animEdPreviewRot{0.0f, 0.0f, 0.0f};
        glm::vec3 m_animEdPreviewScale{1.0f, 1.0f, 1.0f};
        bool m_animEdIsPlaying = false;

        // Preview Viewport State
        unsigned int m_animEdFBO = 0;
        unsigned int m_animEdTex = 0;
        unsigned int m_animEdRBO = 0;
        ImVec2 m_animEdViewportSize{0.0f, 0.0f};

        // --- Asset Import Modal State ---
        std::string m_PendingImportPath = "";
        // bool m_ShowImportModal = false;
        
        bool m_ImportExtractMesh = true;
        bool m_ImportExtractMaterial = true;
        bool m_ImportExtractSkeleton = true;
        bool m_ImportExtractAnimation = true;
